...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($mysqli->query($sql)) {
    ...
    header("Location: offerte_form.php?id=".$newId."&saved=1");
    exit;
  } else {
    if ($isAjax) {
    ...
    header("Location: offerte_form.php".($id?("?id=".$id):"")."&error=".urlencode($mysqli->error));
    exit;
  }
}
?>
...
:root{
  ...
  --mnav-h:64px; /* mobile navbar height */
}
...
.content{padding:22px 26px;height:100dvh;overflow-y:auto;scroll-behavior:smooth;background:#fafafa}
...
#exportWrap{width:var(--doc-w);margin:10mm auto;background:#fff;box-shadow:0 8px 20px rgba(0,0,0,.07);border-radius:12px;overflow-y:auto;overflow-x:hidden;height:calc(100vh - 40px)}
.doc{width:var(--doc-w);min-height:297mm;background:#fff;position:relative;overflow:hidden;padding-bottom:25mm;page-break-after:always;border-radius:12px}
...
.box{padding:10px;background:#fff;box-shadow:0 1px 0 rgba(0,0,0,.02);break-inside:avoid;page-break-inside:avoid;border-radius:10px}
...
@media (max-width:700px){
  ...
  .sidebar{ position:fixed; left:0; top:0; bottom:0; width:min(86vw,320px); transform:translateX(-100%); z-index:10002; transition:transform .25s ease;}
  body.mobmenu-open .sidebar{ transform:translateX(0);}
  .content{
    display:flex; flex-direction:column;
    padding:12px 12px calc(var(--mnav-h) + 12px);
    background:#fafafa;
  }
  #exportWrap{
    margin:8px auto;
  ...
  .toolbar{border-top-left-radius:12px;border-top-right-radius:12px}
  .m-nav{
    position:fixed; left:12px; right:12px; bottom:12px; height:var(--mnav-h);
  ...
  .m-nav button, .m-nav a{
    background:transparent; border:none; padding:10px 8px; border-radius:12px;
...
@media (max-width:700px){
  .mob-overlay{
    position:fixed;inset:0;background:rgba(0,0,0,.25);z-index:10001;
    opacity:0;pointer-events:none;transition:opacity .2s ease;
  }
  body.mobmenu-open .mob-overlay{display:block;opacity:1;pointer-events:auto;}
}
</style>
...
<div class="layout">
  ...
  <div class="mob-overlay" onclick="closeMobileMenu()"></div>
...
<nav class="m-nav" aria-label="Mobile actions">
  ...
  <a href="offerten.php"><span class="ic">📄</span>Alle</a>
  <button type="button" onclick="saveOffer()" class="primary"><span class="ic">💾</span>Sichern</button>
  <button type="button" onclick="downloadPDF()" class="danger"><span class="ic">⬇️</span>PDF</button>
  <button type="button" onclick="openQuickEdit()"><span class="ic">⚙️</span>Edit</button>
</nav>
...
/* ===== Silent AJAX save (used by instant save + before PDF) ===== */
...
function buildPaginatedPages(){
  ...
  while (i < rows.length && pages.length < MAX_EXPORT_PAGES){
    ...
    // Measure available height (approx safe area)
    const tbTop   = tb.getBoundingClientRect().top;
    const pageTop = page.getBoundingClientRect().top;
    const safeHeightPx = 270 * 3.78;
    const limit   = Math.floor(pageTop + safeHeightPx - tbTop);
    ...
    if (added === 0 && i < rows.length){ tb.appendChild(rows[i].cloneNode(true)); i++; }
  }
...
async function downloadPDF(){
  loader(true);
  const saved = await saveOfferAjax().catch(()=>false);
  ...
  try{
    ...
    const name = firstLine(document.getElementById('billTo')?.innerText) || 'Offerte';
    const dateStr = (document.getElementById('offerDate')?.innerText || '').trim() || new Date().toISOString().slice(0,10);
    const filename = `${name} - Offerte - ${dateStr}.pdf`;
...
function openQuickEdit(){ qePanel.classList.add('open'); qePanel.setAttribute('aria-hidden','false'); }
...
/* Sync Quick-Edit <-> document */
function syncFromPanelToDoc(){
  const v = (id)=>document.getElementById(id)?.value || '';
  const setText = (id, val)=>{ const el=document.getElementById(id); if(el){ if(el.tagName==='INPUT' || el.tagName==='TEXTAREA'){ el.value=val; } else { el.textContent = val; } } };

  if (v('qeBillTo')) document.getElementById('billTo').textContent = v('qeBillTo');
  if (v('qeNumber'))  setText('offerNumber', v('qeNumber'));
  if (v('qeDate'))    setText('offerDate', v('qeDate'));
  if (v('qeValid'))   setText('validUntil', v('qeValid'));
  if (v('qeRef')!==undefined) setText('refText', v('qeRef'));
  if (v('qeVat')!==undefined) setText('vatRate', v('qeVat'));
  if (v('qeRemark')!==undefined) document.getElementById('remarkText').textContent = v('qeRemark');
  recalc();
}
function syncFromDocToPanel(){
  const getText = (id)=>document.getElementById(id)?.textContent?.trim() || '';
  document.getElementById('qeBillTo').value = document.getElementById('billTo')?.innerText || '';
  document.getElementById('qeNumber').value = getText('offerNumber') || '00000';
  document.getElementById('qeDate').value   = getText('offerDate') || '';
  document.getElementById('qeValid').value  = getText('validUntil') || '';
  document.getElementById('qeRef').value    = getText('refText') || '';
  document.getElementById('qeVat').value    = getText('vatRate') || (<?= json_encode($company['vat_rate'] ?? '8.1') ?>);
  document.getElementById('qeRemark').value = document.getElementById('remarkText')?.innerText || '';
}
syncFromDocToPanel();

/* Instant save (debounced) */
let saveTimer=null;
function scheduleInstantSave(){
  clearTimeout(saveTimer);
  saveTimer = setTimeout(async()=>{ await saveOfferAjax(); }, 500);
}
['qeCustomer','qeBillTo','qeNumber','qeDate','qeValid','qeRef','qeVat','qeRemark'].forEach(id=>{
  const el=document.getElementById(id);
  if(!el) return;
  el.addEventListener('input', ()=>{ syncFromPanelToDoc(); scheduleInstantSave(); });
  el.addEventListener('change', ()=>{ syncFromPanelToDoc(); scheduleInstantSave(); });
});
document.getElementById('qeSaveBtn')?.addEventListener('click', async()=>{ await saveOfferAjax(); toast('Gespeichert ✓'); });

/* ===== Sidebar mobile toggle ===== */
function toggleMobileMenu(){ document.body.classList.toggle('mobmenu-open'); }
function closeMobileMenu(){ document.body.classList.remove('mobmenu-open'); }
