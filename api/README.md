...
- `GET /api/invoices/:id` — fetch a single invoice for edit screens with the same field names used in list/export.
...
All queries use prepared statements via `mysql2` and validate payloads for required fields, date format, and numeric totals before writing.
